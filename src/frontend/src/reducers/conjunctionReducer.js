...
export const conjunctions = (state = initialState, action) => {
    switch (action.type) {
        ...
        case ADD_CONJUNCTION_SUCCESS:
            return {
                ...state,
                conjunctionsList: state.conjunctionsList.concat(action.response.data[0]),
                loading: false,
        ...
        case MODIFY_CONJUNCTION_SUCCESS:
            ...
            return {
                ...state,
                conjunctionsList: state.conjunctionsList.map(
                    (conjunction) => conjunction.key === updConj.key ? updConj : conjunction),
                loading: false,
        ...
        case DELETE_CONJUNCTION_SUCCESS:
            return {
                ...state,
                conjunctionsList: state.conjunctionsList.filter((conjunction) => conjunction.key !== action.key),
                loading: false,
