...
import { getStats } from "../../actions/ResultActions";
...
class Stats extends React.Component {
  ...
  itemsStatistic(labelPrefix, data) {
    ...
    let success = total - data.failed;
    let percent = ((success / total) * 100).toFixed(2) + "%";
    return [
      {
        key: `${labelPrefix}key`,
        label: `AI ${labelPrefix} Match`,
        value: `${success}/${total}`,
      },
      { key: `${labelPrefix}Accuracy`, label: "AI Accuracy", value: percent },
    ];
  }
  ...
  ruleStatistic(rule) {
    ...
    let success = total - rule.total_failed;
    let percent = ((success / total) * 100).toFixed(2) + "%";
    ...
    return [
      ...
      //   { key: "views", label: "Total AI Wins", value: total },
      {
        key: `${ruleId}-total`,
        label: "AI Matches",
        // value: rule.total_failed,
        value: `${success}/${total}`,
      },
      { key: `${ruleId}-Accuracy`, label: "AI Accuracy", value: percent },
    ];
  }
    ...
    return (
      <>
        ...
        {results &&
          ...
          results.rules.sort(x=>x.rule_id).map((rule, index) => (
            <>
              <Grid.Row centered>
                <Statistic.Group
                  // inverted
