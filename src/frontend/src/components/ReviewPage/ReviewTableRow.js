...
import ReviewTableRowExpansion from "./ResultsRowExpansion/ReviewTableRowExpansion";
import {
  REQUEST_STATES,
  NOTE_CONFIRM_AI,
  BtnTextConfirm,
...
class ReviewTableRow extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
	...
	let hasAi = 
		(result.ai_rule_id != null && result.ai_rule_id !== "")
		|| (result.ai_priority != null && result.ai_priority !== "");
		//|| result.ai_contrast != null;
		//|| result.ai_p5_flag != null;

	let hasLabelled = 
		(result.labelled_rule_id != null && result.labelled_rule_id != "")
		|| (result.labelled_priority != null && result.labelled_priority !== "");
		//|| result.labelled_p5_flag != null;

	let resultsMatch = 
		result.ai_rule_id === result.labelled_rule_id
		&& result.ai_priority === result.labelled_priority
		&& (result.ai_contrast=== null ? false : result.ai_contrast) === (result.labelled_contrast === null ? false : result.labelled_contrast)
		&& (result.ai_p5_flag === null ? false : result.ai_p5_flag) === (result.labelled_p5_flag === null ? false : result.labelled_p5_flag);
    ...
    return (
      <>
        <Table.Row
		  ...
		  positive={hasAi && hasLabelled && resultsMatch}
		  error={hasAi && hasLabelled && !resultsMatch}
        >
		...
		<Table.Cell>{result.ai_rule_id ? result.ai_rule_id : " - "}</Table.Cell>
        ...
        {/*<Table.Cell>{result.final_priority ? result.final_priority : " - "}</Table.Cell>
		<Table.Cell>{result.final_contrast !== null ? result.final_contrast.toString() : " - "}</Table.Cell>*/}
        <Table.Cell>
            <Form.Dropdown
              fluid
              selection
